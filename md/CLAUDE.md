...
- **架构**: 10 个源文件 + `.claude/skills/` 自定义 skill，无子目录，单向依赖
...
E:\study\cctext\
    ...
    weapon.py      — 武器数据定义（剑/枪/斧/弓）
    enemy.py       — BaseEnemy、PatrolEnemy、FlyingEnemy、BossEnemy
    projectile.py  — Arrow 类（弓箭抛物线飞行、碰撞）
    ...
    .claude/
        skills/
            sync-docs.md  — 文档同步 skill（代码变更→更新 rz.md + CLAUDE.md）
```
...
- **文档同步规则**: `.claude/skills/sync-docs.md` 定义的 skill，修改/新增代码后须同步更新 `md/rz.md` 和本文件
